...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int size_impl(const Node *node) {
    ...
    int sizeLeft = size_impl(node->left);
    int sizeRight = size_impl(node->right);
  ...
  static int height_impl(const Node *node) {
    ...
    int leftSubHeight = height_impl(node->left);
    int rightSubHeight = height_impl(node->right);
    
    return 1 + std::max(leftSubHeight, rightSubHeight);
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    newTree->datum = node->datum;
    newTree->left = copy_nodes_impl(node->left);
    newTree->right = copy_nodes_impl(node->right);
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    bool leftSort = check_sorting_invariant_impl(node->left, less);
    bool rightSort = check_sorting_invariant_impl(node->right, less);

    if ((node->right && less(node->right->datum, node->datum)) 
    || (node->left && less(node->datum, node->left->datum))) {
      return false;
    }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    if (less(val, node->datum)) {
      Node* maybe = min_greater_than_impl(node->left, val, less);
