...
// When a Simple Player leads a trick, they play the highest non-trump card in their hand. If they have only trump cards, they play the highest trump card in their hand.
        ...
        Card play_card(const Card &led_card, Suit trump) {
            ...
            for (int i = 0; i < hand.size(); i++) {
                if ((hand[i].get_suit(trump) == led_card.get_suit(trump)) && highestSuitCard == -1) {
                    highestSuitCard = i;
                }
                else if ((hand[i].get_suit(trump) == led_card.get_suit(trump)) && highestSuitCard != -1) {
                    if (Card_less(hand[highestSuitCard], hand[i], led_card, trump)) {
                        highestSuitCard = i;
                    }
                }
                if ((hand[i].get_suit(trump) != led_card.get_suit(trump)) && lowestCard == -1) {
                    lowestCard = i;
                }
                else if ((hand[i].get_suit(trump) != led_card.get_suit(trump)) && lowestCard != -1) {
