...
class Game {
    private:
        ...
        Suit make_trump(Card& upcard, int& trumpMakingPlayer) {
            ...
            if (!trumpMadeRound1) {
                for (int i = (dealerPos+1) % 4; 
                i < ((dealerPos+1) % 4)+4 
                && !trumpMadeRound1; i++) {
                    // dealer index
                    if (i % 4 == dealerPos) {
                        trumpMadeRound1 = 
                        players[i%4]->make_trump(upcard, 
                        true, 2, orderUpSuit);
                    }
                    // non dealer index
                    else {
        ...
        void play_hand(Suit& trumpSuit, int trumpMakingPlayer) {
            ...
            for (int trick = 0; trick < 5; trick++) {
                ...
                for (int person = leader; 
                person < leader+4; person++) {
                    int player = person%4;
                    if (player == leader) {
                        leadCard = 
                        players[player]->lead_card(trumpSuit);
                        cards[player] = leadCard;
                        cout << leadCard << 
                        " led by " << *players[player] << endl;
                    }
                    else {
                        cards[player] = 
                        players[player]->play_card(leadCard, 
                        trumpSuit);
                ...
                // by this point all 4 choices of cards 
                // played by the 4 players is stored in the cards array
                ...
                cards[3], leadCard, trumpSuit)) {
                    winningCardIndex = 3;
                    // winningCardIndex now contains the 
                    // player's index with the most valuable card 
                    // (the winner for this trick)
                }
                
                // figuring out if team1 
                // or team2 won the trick based on winningCardIndex
                ...
                cout << *players[winningCardIndex] 
                << " takes the trick\n\n";
                leader = winningCardIndex;
                
            }
            // at this point 5 tricks are complete, 
            //hence 1 hand is complete, printing winner of hand now
            ...
            // calling updateScore function 
            // to update team scores and determine whether there is a march, euchre
            updateScore(trumpMakingTeam, 
            team1TrickWins, team2TrickWins);
 ...
 public:
  ...
  void play() {
    cout << endl;
    while (team1Points < points_to_win 
    && team2Points < points_to_win) {
        // start of a hand
        shuffleHand();
