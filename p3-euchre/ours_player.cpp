...
// When a Simple Player leads a trick, they play the highest non-trump card in their hand. If they have only trump cards, they play the highest trump card in their hand.
        ...
        Card play_card(const Card &led_card, Suit trump) {
            ...
            for (int i = 0; i < hand.size(); i++) {
                if (hand[i].get_suit(trump) == led_card.get_suit() && highestSuitCard == -1) {
                    highestSuitCard = i;
                }
                else if (hand[i].get_suit(trump) == led_card.get_suit() && highestSuitCard != -1) {
                    if (Card_less(hand[highestSuitCard], hand[i], led_card, trump)) {
                        highestSuitCard = i;
                    }
                }
                if (hand[i].get_suit(trump) != led_card.get_suit() && lowestCard == -1) {
                    lowestCard = i;
                }
                else if (hand[i].get_suit(trump) != led_card.get_suit() && lowestCard != -1) {
                    if (Card_less(hand[i], hand[lowestCard], led_card, trump)) {
                        lowestCard = i;
                    }
                }
            }
...
class HumanPlayer : public Player {
    ...
    public:
        ...
        bool make_trump(const Card &upcard, bool is_dealer,
            ...
            else if(string_to_suit(decision) == upcard.get_suit()) {
                order_up_suit = upcard.get_suit();
                return true;
            }
            else{
