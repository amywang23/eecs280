...
class Game {
    private:
        ...
        void play_hand(Suit& trumpSuit, int trumpMakingPlayer) {
            ...
            int leader = ((handNum+1) % 4); // after first round becomes winner of previous trick
            // int team1TrickWins = 0; // player 0 and player 2
            // int team2TrickWins = 0; // player 1 and player 3
            for (int trick = 0; trick < 5; trick++) {
                Card cards[4]; // indexes corresponding to player0,1,2,3 and the cards they played
                Card leadCard;
                ...
                for (int person = leader; person < leader+4; person++) {
                    ...
                    if (player == leader) {
                        ...
                        cards[player] = players[player]->play_card(leadCard, trumpSuit);
                        cout << cards[player] << " played by " << *players[player] << endl;
                ...
                if (Card_less(cards[0], cards[1], leadCard, trumpSuit)) {
                    winningCardIndex = 1;
                }

                // comparing the highest card so far to player2's card
                if (Card_less(cards[winningCardIndex], cards[2], leadCard, trumpSuit)) {
                    winningCardIndex = 2;
                }

                // comparing the highest card so far to player3's card
                if (Card_less(cards[winningCardIndex], cards[3], leadCard, trumpSuit)) {
                    winningCardIndex = 3;
            ...
            // calling score function to update team scores and determine whether there is a march, euchre
